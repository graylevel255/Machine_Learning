...
function params_single_class = train_model_single_class(class_data, val_set_class_data, no_of_classes)
    ...
    for i=1 : size(Q,2)
        ...
        [w,mu, sigma] = get_initial_estimates_nb(class_data, q);
       
        
          while true
%           disp('print here initial');
%           if(q == 3)
%           mu
%           sigma
%           w
...
%           disp('print here after');
%           if(q == 3)
%                 mu
%               sigma
%               w
%           end
            %for Naive Bayes, make off-diagnoal entries 0
 ...
 function [w,mu, sigma] = get_initial_estimates_nb(class_data, q)
    ...
    for c_index = 1:q
        ...
        w = [w sum(p)/n];
        %to check clustering just plotting data
        %if (q == 4)
        %  scatter(single_cluster_data(:,1), single_cluster_data(:,2), sym(c_index))
        %   hold on;
       
        %end
    end
    %if (q == 4)
    %    title('Scatter plot to verify k-means clustering, k =4');
    %    hold off;
    %end
    mu = means;
 ...
 function c = calc_cov_with_resp_nb(A, mean, resp_term) %verify this with alt impl.
    n = size(A,1);
    d = size(A,2);
    sum1 = zeros(d,d);
    for i = 1:n
    ...
    c = sum1;
    %make off diag zero for naive bayes
    for i=1:d
        for j=1:d
            if(i ~= j)
                c(i,j) = 0;
 ...
 function [w,mu,sigma] = calc_new_params(resp_term,class_data,q)
    ...
    w = [];
            for j = 1:q
                ...
                c_q = calc_cov_with_resp_nb(class_data, mu_q, rnq);
